...
(function() {
    ...
    console.log('📋 Scripts with data-webhook found:', scripts.length);
    
    if (scripts.length > 0) {
        WEBHOOK_URL = scripts[scripts.length - 1].getAttribute('data-webhook');
        console.log('✅ Found webhook via data-webhook attribute:', WEBHOOK_URL);
    }
    ...
    if (!WEBHOOK_URL) {
        ...
        for (var i = allScripts.length - 1; i >= 0; i--) {
            ...
            if (script.src && script.src.includes('universal.min.js')) {
                ...
                if (webhook) {
                    WEBHOOK_URL = webhook;
                    console.log('✅ Found webhook on universal script:', WEBHOOK_URL);
                    break;
                }
            }
        }
    ...
    if (!WEBHOOK_URL) {
        ...
        console.log('⚠️ Using fallback webhook URL:', WEBHOOK_URL);
    }
    
    console.log('🎯 Final webhook URL:', WEBHOOK_URL);
    ...
    function captureFormData(container) {
        console.log('📋 captureFormData called with container:', container);
        var data = {};
            ...
            if (input.value && input.value.trim() !== '' && !input.disabled) {
                ...
                if (input.type === 'checkbox' || input.type === 'radio') {
                    if (input.checked) {
                        ...
                        console.log('✅ Captured checkbox/radio:', key, data[key]);
                    }
                } else if (input.tagName === 'SELECT') {
                    var selectedOption = input.options[input.selectedIndex];
                    if (selectedOption && selectedOption.value) {
                        data[key] = selectedOption.value;
                        console.log('✅ Captured select:', key, data[key]);
                    }
                } else if (input.type !== 'submit' && input.type !== 'button') {
                    data[key] = input.value;
                    console.log('✅ Captured input:', key, data[key]);
                }
            } else {
                console.log('⚠️ Skipped input (no value or disabled)');
            }
        });
        ...
        if (Object.keys(adTracking).length > 0) {
            ...
            console.log('📊 Added tracking params:', adTracking);
        }
    ...
    function findRelevantContainer(element) {
        console.log('🎯 findRelevantContainer called with:', element);
        ...
        while (parent && parent !== document.body) {
            ...
            if (inputsInParent.length > 0) {
                console.log('✅ Found parent container with inputs');
                return parent;
            }
            parent = parent.parentElement;
        }
        
        console.log('⚠️ Using document as fallback');
        return document;
    }
    
    function sendToWebhook(data, context) {
        console.log('📡 sendToWebhook called with context:', context);
        ...
        console.log('🔍 Form data (excluding meta):', formData);
        console.log('🔍 Has actual form data:', hasActualFormData);
        
        if (!hasActualFormData) {
            console.log('❌ No actual form data - only metadata. Skipping webhook.');
            return false;
        }
        ...
        console.log('⏰ Time difference:', now - lastSubmissionTime);
    ...
    document.addEventListener('click', function(e) {
        ...
        console.log('🎯 Is submit element:', isSubmitElement);
        
        if (isSubmitElement) {
            console.log('✅ Submit element clicked - processing...');
            ...
            if (hasFormFields) {
                console.log('✅ Proceeding with webhook call');
                sendToWebhook(data, 'form_click');
            } else {
                console.log('⚠️ No form fields found - skipping webhook');
            }
        }
    });
    ...
    console.log('✅ Universal script loaded and event listeners attached');
})();
